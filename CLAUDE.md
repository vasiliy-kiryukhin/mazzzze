...
└── HUD (CanvasLayer)
    ...
    ├── PhotoEnter (Control + PhotoEnterHud.cs) — photo walk-into live window + yellow transition flicker
...
- **Vintage camera** (`REQ-0013-vintage-camera`, US-13): **LMB** (`use_activated`) on the activated camera opens `ViewfinderHud` — a framed **window above the player's head** (third-person view kept, **no darken**) showing a `SubViewport` first-person **level/yaw** lens view (shared `World3D`), with sepia + vignette + countdown 3→2→1 (`TickSeconds` 0.6667 — 3× faster). Window is small — `WindowWidthFraction` 0.15 (halved) to barely occlude the scene. Focus check = forward horizontal ray, min 1.8 (3×0.6); blocked-before or lost-during resets without consuming. Mouse pitch stays free (third-person). On fire → `PhotoItem` created into the reserved slot, camera destroyed.
- **Photo** (`REQ-0017-photo`, US-17): activated photo shows a **live, full-colour window centred on screen with a simple drawn frame** (`PhotoEnterHud` — one `SubViewport` `_vp` renders the captured point via a `Camera3D` at `CapturedWorldPos`/yaw, so a monster passing there shows live; procedural wood/brass border in `_Draw`; **monochrome + sepia removed**). The photo **comes alive** as you enter: the preview `Camera3D.Environment` carries a **warm ambient fill** (`AmbientLightSource=Color`, HeadLight colour) whose energy is ramped by entry progress in `_Process` (`AmbientLightEnergy = Lerp(AmbientEnergyMin 0.2, AmbientEnergyMax 1.6, Progress)`) — dim "sleeping" shot at progress 0, fully lit by teleport, so no dark-to-bright jump. A real `OmniLight3D` can't be used (shared `World3D` would leak the light into the main view); the per-camera Environment stays local. A 3D polaroid-frame variant (from a `polaroid_photo.glb` model) was tried and dropped, and the model removed. Walking forward teleports: `UpdatePhotoEnter` accrues progress while `move_forward` held and actually advancing (`Velocity·camForward > Speed*0.4`), the window **grows** from centre; at `EnterDuration` 1.3333 s (1.5× shorter) → `Player.TeleportTo(pos, yaw, pitch)` (main top-down pitch preserved), a **light transparent yellow flicker** (`Flash()`: `FlashMaxAlpha` 0.35 × decaying sine, `FlashFlickers` 3) masks chunk streaming, photo consumed. Camera fire also plays `PlayPickupGesture()` + slot flash so the new photo is noticed. Live view is limited to currently-streamed chunks.
- **Tennis ball** (`REQ-0021-tennis-ball`, US-21): first **reusable** item (never consumed) and first **real physical projectile**. Plain `Item` (`TypeId "tennis_ball"`, Usage=ActivatedB, `art/base_ball.glb`), seeded in slot 1. Activated → ball shows in hand via **`BoneAttachment3D` on the `RightHand` bone** (`Player.ShowHandItem`; follows the animation at any camera angle — a fixed `ModelPivot` offset looked detached and was replaced). **Hold LMB** (`use_activated`) charges (`InventoryHud.UpdateThrowCharge`: `_chargeT` plateaus at `MaxChargeTime`, slows player via `Player.ExternalSpeedFactor`, pulses hand model); **release** throws (`ThrowActivated`: force = lerp(`MinThrowForce`,`MaxThrowForce`); dir = `Player.CameraYawForward`; spawned at `Player.HandThrowOrigin` so it leaves the hand, not the back); **RMB** (`throw_cancel`) aborts. Throw is `Activated → InWorld` — slot frees like `DropActivated` but item is **not** consumed (rides the projectile, returns via auto-pickup). `ThrowProjectile` is a **kinematic `CharacterBody3D`** on `MoveAndCollide` (not `RigidBody3D` — that tunnelled through the thin floor/walls, floated on engine gravity 9.8, bounced endlessly in the narrow corridor): manual `Gravity` 15 (game-matched), `LinearDamp`, **normal-reflection** bounce (`Restitution` on normal, `ImpactFriction` on tangential), `GroundFriction` roll-friction on floor for a crisp stop (no creep), swept → no tunnelling. Layer 0 / mask 1. **Monster hit is by proximity, not layer** (monsters share layer 1 with walls): `NearestMonsterHit` vs `Monster.All`; if the monster was in the throw's `aware` set (built from `Monster.SeesPlayerNow()` at release) → `Monster.Stun()` (F-49 branch A), then `Land()` (one target, no piercing). Stop / `MaxAirborneTime` → `Land()` spawns a `WorldItem` at `(x, y−radius, z)` (resting height → seamless handoff, no sink/jerk). The ball uses a single `BallSize` in hand/flight/world (`WorldHeightFor`) so it never resizes on landing. **Distraction (branch B) needs no ball code** — the settled `WorldItem` is picked up by the monster's existing `FindVisibleLure`/`Distract`.
...
- **Not implemented / hooks**: `Stun()` is triggered by the tennis-ball throw (REQ-0021) on a direct hit by a monster that saw the throw; `SeesPlayerNow()` exposes perception for the aware-at-throw check. Distraction reacts to any `WorldItem` (no dedicated lure type) — the settled thrown ball uses this. Ranged delivery + Small size are future; player health is a future feature (monster only reports hits); no `Death` clip hooked (no death state yet). The old `Mob.cs`/`mob.tscn` charge stub is **superseded** (still present, unused).

### Input map

WASD / arrow keys, **Tab** (minimap orientation), **I** (inventory, single press), **1–4** (inventory row/cell → apply/activate), **Shift+1–4** (drop, incl. drop-activated on the reserved cell), **G** (drop cursor slot), **LMB** (`use_activated` — use activated camera; **hold** to charge the tennis-ball throw, release to throw), **RMB** (`throw_cancel` — abort a ball charge), **hold W/↑** (walk into activated photo), **Ctrl+Q** quits, mouse look, wheel zoom, gamepad left stick. Pickup is automatic (no key). See `[input]` in `project.godot`.
...
| `art/base_ball.glb` | **Active** | Tennis-ball item model (US-21); in inventory icon, in hand, as flying projectile, and as world item |
