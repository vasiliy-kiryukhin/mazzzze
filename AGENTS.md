...
- **Item system** hubs in `InventoryHud.cs` (`HUD/Inventory`) — it owns the item state machine (InWorld / InInventory / Activated). `Inventory`/`Item` = 12-slot model; `Item.Usage` is ImmediateA (`Use()`) or ActivatedB (into hand). Slot icons render `Item.BuildModel()` (glb, or a procedural polaroid for `PhotoItem`) into a `SubViewport`; same factory builds the in-world model (scale = `WorldItemSizeFraction` 0.25 × player height). Drop (`DropProjectile`) flings a "star" that lands as a `WorldItem` (static `WorldItem.All` registry). Pickup is **automatic** (no key): scan registry for nearest armed item in range with line-of-sight; `PickupProjectile` flies it back. `ItemStar` is the shared star visual.
  - **Activation / reservation** (F-18/B, F-19a): a pattern-B item stays in its slot but is flagged `_activatedItem`/`_reservedSlot`, blocking the slot; `ActivateSlot`/`Deactivate`/`DropActivated`/`ConsumeActivated` transition it. Activation plays `Player.PlayPickupGesture()` (the `Interact` clip). **Vintage camera** (REQ-0013): **LMB** (`use_activated`) opens `ViewfinderHud` — a framed **window above the player's head** (third-person view kept, no darken) showing a `SubViewport` first-person level/yaw lens view, sepia + vignette + 3→2→1 timer (`TickSeconds` 0.6667, 3× faster), forward focus ray min 1.8; window is small (`WindowWidthFraction` 0.15, halved). On fire a `PhotoItem` is created into the reserved slot and the camera destroyed. On fire it also plays `PlayPickupGesture()` + a slot flash so the new photo is noticed. **Photo** (REQ-0017): activating it opens a **live, full-colour window centred on screen with a simple drawn frame** (`PhotoEnterHud` — one `SubViewport` `_vp` renders the captured pos/yaw; **monochrome + sepia removed**; the photo **comes alive** as you enter — the per-camera `Environment` carries a **warm ambient fill** (HeadLight colour) whose energy is ramped by entry progress in `_Process` (`Lerp(AmbientEnergyMin 0.2, AmbientEnergyMax 1.6, Progress)`): dim "sleeping" shot at progress 0, fully lit by teleport, no dark-to-bright jump — a real `OmniLight3D` would leak through the shared `World3D`; procedural wood/brass border drawn in `_Draw`; passing monsters show live; a 3D `polaroid_photo.glb` frame variant was tried, dropped, and the model removed); **holding W** while advancing grows the window from centre and teleports at `EnterDuration` 1.3333 s (1.5× shorter) (`UpdatePhotoEnter` → `Player.TeleportTo`, main pitch preserved) + a **light transparent yellow flicker** (`Flash()`, `FlashMaxAlpha` 0.35 × decaying sine, `FlashFlickers` 3). **Tennis ball** (REQ-0021): first **reusable** item + first **real physical projectile**. Plain `Item` (`TypeId "tennis_ball"`, ActivatedB, `art/base_ball.glb`), seeded slot 1. Activated → ball in hand via **`BoneAttachment3D` on the `RightHand` bone** (`Player.ShowHandItem`; follows the animation at any camera angle — a fixed `ModelPivot` offset looked detached and was replaced). **Hold LMB** (`use_activated`) charges (`UpdateThrowCharge`: `_chargeT` plateaus at `MaxChargeTime`, slows player via `Player.ExternalSpeedFactor`, pulses hand model); **release** throws (`ThrowActivated`: force = lerp(`MinThrowForce`,`MaxThrowForce`), dir = `Player.CameraYawForward`, spawned at `Player.HandThrowOrigin` so it leaves the hand not the back); **RMB** (`throw_cancel`) aborts. Throw = `Activated → InWorld` — slot frees like `DropActivated` but item is **not** consumed (rides the projectile, returns via auto-pickup). `ThrowProjectile` is a **kinematic `CharacterBody3D`** on `MoveAndCollide` (not `RigidBody3D` — that tunnelled through the thin floor/walls, floated on engine gravity 9.8, bounced endlessly in the narrow corridor): manual `Gravity` 15 (game-matched), `LinearDamp`, **normal-reflection** bounce (`Restitution` on normal, `ImpactFriction` on tangential), `GroundFriction` roll-friction on floor for a crisp stop (no creep), swept → no tunnelling. Layer 0 / mask 1. **Monster hit by proximity, not layer** (monsters share layer 1 with walls): `NearestMonsterHit` vs `Monster.All`; if the monster was in the throw's `aware` set (from `Monster.SeesPlayerNow()` at release) → `Monster.Stun()` (F-49 branch A), then `Land()` (one target, no piercing). Stop / `MaxAirborneTime` → `Land()` spawns a `WorldItem` at `(x, y−radius, z)` (resting height → seamless handoff, no sink/jerk). The ball uses a single `BallSize` in hand/flight/world (`WorldHeightFor`) so it never resizes on landing. Distraction (branch B) needs no ball code — the settled `WorldItem` is picked up by the monster's existing `FindVisibleLure`/`Distract`.
  - Requirements: `REQ-0011-inventory/`, `REQ-0012-base-item/` (+ sub-features `REQ-001{4,5,6}-...`), `REQ-0013-vintage-camera/`, `REQ-0017-photo/`, `REQ-0021-tennis-ball/`. Not implemented: `Item` as `.tres` type registry, serialization, edge-screen activated indicator, photo thumbnail icon; ball aim-arc / trail / in-flight distraction.
- **Monster system** (`REQ-0019`/`REQ-0020`): `Monster` (`src/Monster.cs`, abstract `CharacterBody3D`) is the template; `Ifrit` (`src/Ifrit.cs`) the first concrete type (fiery humanoid ifrit, contact). Static `Monster.All` registry (persistent under `Main`, not chunk-bound). Perception = vision cone + LoS ray (`CanSee`); FSM in `_PhysicsProcess` — `Cycle` (patrol) / `Threat` (chase) / `Stun` / `Distract`, priority Stun > player-visibility > distraction, no memory after disruption. Movement = **BFS over `MazeData.IsFloor` cells** (patrol restricted to a segment ≈ 1 chunk); model faces movement via `LookAt` + `ModelYawOffsetDeg` (180° — ifrit forward is +Z like the player rig). Animation (`UpdateAnim`/`PlayAttack`) plays the glb's `AnimationPlayer` clips: `IdleAnim`/`MoveAnim` (looped via `SetLoop`, speed-scaled) + `AttackAnim`/`StunAnim` (one-shot); the ifrit ships a full set — `Idle`/`Run`/`Attack`/`BeHit` (`Monster_YiFuLiTe_*`). Contact damage emits `PlayerHit` signal + `DamageHud` red flash (no health system yet). Model scaled by **height** (`TargetHeight`; `ScaleByLength` for low/long models), local-space AABB (local transform chains, not global coords — was needed against float32 loss at world −18000, still sound near the origin). **NB:** if `BuildBody` can't load `ModelPath` (e.g. `.godot/imported/*.scn` missing because assets weren't re-imported) it falls back to an *empty* `Node3D` — the monster then has collision + contact damage but no visible mesh; run `--import` to fix. `MonsterSpawner` (`Main/MonsterSpawner`) is a **minimal debug spawner** near start (real spawner is future). Hooks/TODO: `Stun()` is triggered by the tennis-ball throw (REQ-0021) on a direct hit by a monster that saw the throw (`SeesPlayerNow()` exposes perception for the aware-at-throw check); distraction reacts to any `WorldItem` (the settled thrown ball uses this); Ranged/Small/health/death-state are future.
